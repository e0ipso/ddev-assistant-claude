...
- `config.assistant-claude.yaml` — DDEV hooks: **pre-start** (`exec-host`) seeds `.ddev/claude-code/.claude/` and `.ddev/claude-code/.claude.json` from the host user's `~/.claude/` and `~/.claude.json` the first time only (skipped if the project store already exists; `.claude.json` falls back to an empty `{}` if the host has none, since Docker requires a bind-mounted file to pre-exist); **post-start** (`exec`) fixes ownership on the mounted store, locks down credential permissions, and symlinks the container's `~/.claude` and `~/.claude.json` to the store (must run in-container so `$HOME` resolves to the container's home, not the host's)
- `docker-compose.assistant-claude.yaml` — Bind-mounts the project-local `.ddev/claude-code/.claude/` directory and `.ddev/claude-code/.claude.json` file read-write to fixed container paths (`/home/.claude-project-store`, `/home/.claude-project-store.json`); fixed rather than `$HOME`-relative because `$HOME` is interpolated on the host at compose-render time and can differ from the container's `$HOME` (e.g. macOS). `~/.claude.json` matters because it (not just `~/.claude/.credentials.json`) holds the OAuth account/session identity — `machineID`, `userID`, project trust state — that Claude Code checks alongside the token; leaving it unmounted gives the container a fresh identity on every restart
